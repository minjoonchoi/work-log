...
- Choose `required: false` only when `review_policy.omission_allowed` is true **and** the result is a bounded, low-impact text transformation using supplied facts and a clear output format, without new factual claims, product decisions, or implementation. The current catalog allows this only for `text.generate`; a short acknowledgement, wording alternative, or format conversion can qualify. Example reason: “제공된 문구를 같은 의미로 정리하며 새 사실·설계 판단을 추가하지 않습니다.”
