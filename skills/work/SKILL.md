...
This entry skill is optional. WorkLog's connection settings manage history collection separately from harness delegation. Use it only while harness delegation is connected for the requesting engine. The catalog exposes the installed job packages, not every job bundled with the app. If delegation is disconnected or a required job package is not installed, explain which connection/package is needed; do not reconnect, install packages, unset the native origin, or substitute a generic job to evade that choice. Existing accepted work can finish after disconnection or package removal.

WorkLog's own title/description generation, session summaries, calendar work summaries, Jira result summaries, and task-type drafts are app management jobs. They remain available independently of harness packages and must not be selected as substitutes for delegated user work. Configure their instructions and models in **WorkLog 자동 생성**, separately from **하네스 작업**.
...
3. Identify requested outcomes and operations, then assign one task type to each. A source noun is not a requested outcome: “이 PRD를 바탕으로 API를 설계” requests API design, not a new PRD. “이 API 설계를 검토만” requests review, not design, implementation, or repair of the source. Inspecting a source document is the primary work of `document.review`; it does not request another reviewer for the resulting report and never authorizes changes to the source.
...
- Use the default `required: false` for `document.create`, `document.share.create`, `document.update`, `document.review`, `text.generate`, factual `meeting.summarize` / `progress.summarize`, and registered types inheriting those templates. They produce their requested result once and apply code checks, without automatic independent review or repair. Set `required: true` only if the user additionally requests an independent quality review of that result.
- “이 문서를 검토해 줘” selects `document.review` with `required: false`: inspecting the original document and writing findings is the requested work, not an instruction to review that report again. Example reason: “요청한 원본 문서 검토를 한 번 수행하고 결과 형식을 검사하며, 보고서의 별도 품질 검토는 추가하지 않습니다.” “검토 보고서도 별도 검토자가 검증해 줘” explicitly opts into its additional quality gate.
- Preserve mandatory `required: true` for specialized PRD, design, code, research, and professional review tasks where `omission_allowed` is false. A short prompt, few files, time pressure, or a desire to reduce cost does not justify relabeling their deliverable as `document.create` or `text.generate`. Decide each step separately in a mixed request.
- Review-only tasks still inspect their requested subject and return findings; the quality gate never converts that task into a plain summary or authorizes edits to its subject. A completed review report does not mean the reviewed subject passed. Follow the current catalog for newly registered types rather than inferring their policy from a label.
...
Group tightly coupled changes into one step. Do not split by method, file count, role name, or workflow stage. The runtime performs verification and the selected workflow's review/repair gates, so do not add duplicate plan/review/test/repair steps unless they are separately requested deliverables. A request for one document is one creation step, with its review decision; it is not a plan → create → review → repair plan. A separately requested implementation plan or review report can be its own output. Use existing supplied PRDs, designs, and contracts directly instead of scheduling their recreation. Parallel tasks must own distinct outputs; shared decisions are predecessors only when requested or necessary within an owning task's boundary. If no task can safely own a prerequisite, resolve that gap before submission.
...
## Recover an accepted execution

An agent stage has a one-hour timeout in new executions; this is not a deadline for the whole plan. Generation, review, and each permitted repair have their own stage budget. Use the limits frozen in the accepted execution; do not assume that changing installed defaults updates an existing run.

If waiting is interrupted, inspect the accepted plan with `status <plan-id>`. A CLI wait or HTTP connection error does not prove that its worker stopped. Keep waiting while it is pending or running; do not create another plan/run or change its idempotency key.

For a failed, blocked, or interrupted run, inspect its failed stage, attempts, failure reason, and available candidate/verification evidence before deciding to resume. When the cause is resolved and continuation is within the authorized scope, use `resume <plan-id> --wait` (or the original run ID for a standalone run). The runtime verifies saved inputs and candidates and resumes the retained stage; a review timeout must not become an instruction to generate the document again. Do not manually copy partial files, skip a failed gate, or create a replacement run to bypass a recovery, version, integrity, or budget rejection. If no trustworthy checkpoint is available, report that limitation before starting generation again. A repeated failure without a new remedy ends recovery rather than starting a retry loop.

GUI summary/item-rewrite failures remain under the writing service's scheduling policy. Do not start hidden background retries, duplicate text tasks, or fresh runs for the same source snapshot. A deliberate regeneration request or the next eligible source event is distinct from silently retrying the failed request.
...
Use `status|result <plan-id>` to inspect a plan, and `evidence <run-id>` for a registered local check's evidence. Use `cancel <plan-id>` within the user's authorized intent; apply the recovery procedure above before `resume`. Hook activity alone is not proof of execution.
