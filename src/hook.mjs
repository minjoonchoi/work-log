...
import { dataRoot, initRoot, database, transaction, id, stableId, now, json, atomic, redact } from './shared.mjs';
...
try {
  ...
  if (kind && raw.session_id) {
    const internal = process.env.HARNESS_WORKER === '1';
    // Executor owns worker I/O. Hooks add lifecycle/tool evidence, never duplicate its prompt/output.
    if (!(internal && ['input', 'output'].includes(kind))) {
      ...
      const event = transaction(db, () => {
        ...
        const parent = internal ? JSON.parse(process.env.HARNESS_PARENT || '{}') : undefined;
        ...
        const e = { id: raw.event_id || id('hook-'), engine,
          agent_session_id: internal ? process.env.HARNESS_ATTEMPT_ID || raw.session_id : raw.session_id,
          source_session_id: raw.session_id, kind, event_at: stamp, observed_at: stamp, time_source: 'hook_observed',
          turn_id: turn, call_id: raw.tool_use_id || raw.tool_call_id || null, role: internal ? 'worker' : 'user',
          ...(parent ? { parent, work_item_id: parent.work_item_id } : {}),
