...
export function reviewPolicy(task, job, workflow) {
  ...
  return { default_required: required, omission_allowed: task === 'text.generate' && job.kind === 'document' && !job.allow_internal,
    scope: task === 'text.generate'
...
export function compileReview(task, job, workflows, selection) {
  ...
  if (selection !== undefined) {
    ...
    assert(selection.required || policy.omission_allowed, '이 업무는 독립 검토를 생략할 수 없습니다. 단순 text.generate에만 review.required=false를 지정하세요.');
  }
  ...
  return { workflow_id: selection?.required === false ? 'create-checked' : job.workflow, decision: { ...decision } };
}
