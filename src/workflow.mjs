...
export function validateWorkflow(workflow, taskTypes) {
  ...
  for (const [id, node] of Object.entries(workflow.nodes)) {
    ...
    for (const [outcome, next] of Object.entries(node.on)) {
      ...
      if (outcome === 'revise') assert(workflow.nodes[next]?.task === 'repair' && workflow.nodes[next].budget === 'repairs', '수정 요청은 제한된 repair로만 연결해야 합니다.');
  ...
  // Artifact workflows must preserve verify → independent review → publish for every version.
  if (workflow.mode === 'artifact') {
    ...
    for (const node of Object.values(workflow.nodes)) {
      ...
      if (node.task === 'verify') assert(next === 'review', '검증 후 독립 검토를 건너뛸 수 없습니다.');
