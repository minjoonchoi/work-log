...
test('scroll loads older I/O pages; multi-page live catch-up preserves the reading position and every stored record', async ({ page }) => {
  ...
  const anchor = session.locator('.event').nth(25), anchorId = await anchor.getAttribute('data-event-id');
  ...
  await session.locator('.conversation-heading').evaluate(el => el.scrollIntoView({ block: 'start' }));
...
test('failed older-page load retains records and supports an explicit retry without duplicate rows', async ({ page }) => {
  ...
  await open(page); await page.locator('.session-card > summary').click();
...
test('a late Stop that splits a session removes moved records from an already open history', async ({ page }) => {
  ...
  await page.locator('.session-card > summary').click(); await expect(page.locator('.event')).toHaveCount(3);
...
test('switching work items while a history request is delayed ignores the old response', async ({ page }) => {
  ...
  await open(page, '이전 업무'); await page.locator('.session-card > summary').click(); await waiting;
  await page.getByRole('button', { name: '다른 업무', exact: true }).click(); await page.locator('.session-card > summary').click();
