...
- GUI의 **연결 설정**에서 Claude와 Codex의 **이력 수집**과 **하네스 위임**을 각각 연결·해제합니다. 이력 수집은 `UserPromptSubmit`, `Stop` 등 시스템 훅만, 하네스 위임은 요청용 `work` 스킬만 연결합니다. 하네스 없이도 업무·세션 수집과 자동 요약을 사용할 수 있습니다. 앱 설치만으로 외부 에이전트 설정을 변경하지 않습니다.
- **직무별 하네스 작업**에서 PM·PO·프런트엔드·백엔드·공통 유형을 설치·제거합니다. 신규 설치에서는 필요한 묶음만 선택하며 기존 사용자의 전체 유형과 설정은 유지합니다. 제거는 새 위임만 제한하고 접수된 실행·이력·사용자 등록 유형은 보존합니다. 작업 실행 설정도 **WorkLog 자동 생성 / 하네스 작업**으로 분리합니다. [선택형 하네스와 패키지](docs/harness-packages.md)
- 업무는 **첫 사용자 프롬프트가 수집될 때** 생성합니다. Codex·Claude를 열기만 하면 생성하지 않으며, 앞서 받은 시작·종료·도구·출력 이벤트는 보관했다가 첫 입력의 업무에 연결합니다. 연결 전에 시작한 에이전트 세션도 `SessionStart` 없이 첫 입력부터 업무와 세션 이력을 만듭니다. 연결 설정은 **훅 수신 대기 / 수신 확인**을 별도로 표시합니다. Codex는 `/hooks`에서 추가된 WorkLog 훅의 신뢰를 승인해야 실행할 수 있습니다. 실행 중인 환경이 설정을 다시 읽지 않으면 재개가 필요할 수 있으며, WorkLog가 수신하지 못한 연결 전 원문은 복원하지 않습니다. [중간 연결과 수집](docs/agent-connection-collection.md)
...
- WorkLog 관리 서비스가 1초마다 미요약 세션을 확인하고 최대 5개를 백그라운드로 처리합니다. 다음 구간이 생긴 종료 세션과 마지막 응답 후 20분 이상 유휴한 현재 세션이 대상이며, 응답·사용자 요청 실행이 진행 중이면 제외합니다. 새 프롬프트 없이도 남은 이력을 처리하고 대기·실행·종료 미확인 합계는 5개 이하로 유지합니다(실제 worker 한도 3개). 같은 원문의 실패는 자동 반복하지 않으며 **재요약** 또는 원문 변경으로 다시 처리합니다. 유휴 요약은 세션이나 업무를 완료 처리하지 않습니다.
...
- PM·PO·FE·BE의 사용자 업무 63개와 시스템 업무 7개. 기본 실행은 **독립 검토 56종, 생성 1회·코드 검사 12종, 로컬 실행 2종**입니다. 업무마다 담당 산출물·제외 범위·필요 자료·완료 기준을 고정하고, 지정된 workflow의 게이트와 허용된 최대 2회 수정을 적용합니다. [전체 작업 카탈로그와 경계](docs/job-catalog.md)
...
- 별도 subprocess인 `codex exec`·`claude -p` 어댑터, 실행 동시성 3, 모델 단계별 **1시간** timeout·출력량 제한, 프로세스 트리 취소, 중단 복구, 게시 직후 crash 복구. 검토 실패 후 명시적으로 재개하면 저장한 생성 결과를 확인하고 검토 단계부터 이어갑니다. 수정 라운드 한도는 재개해도 초기화하지 않습니다. [70종 실행·왕복 기준](docs/task-execution-matrix.md)
...
- GUI 텍스트·회의/진행 사실 요약과 `document.*` 4종·`text.generate`는 기본적으로 **요청 결과 직접 생성 1회 → 서비스 저장·코드 검사**로 끝냅니다. 일반 문서·텍스트의 독립 검토·수정은 추가로 요청한 경우에만 수행합니다. `document.review`는 원본 문서를 검토하는 본 업무이며 작성한 보고서의 재검토를 자동 추가하지 않습니다. 전문 PRD·설계·코드·조사 업무의 필수 검토는 유지합니다. 기본 Codex는 **`gpt-5.6-luna` / `high`**입니다. [작업별 실행 한도와 검증 범위](docs/bounded-orchestration.md)
...
처음 사용할 때 **연결 설정 → 이력 수집 연결**만 선택하면 기록과 자동 생성 기능을 사용할 수 있습니다. 작업 위임도 필요하면 해당 에이전트의 **하네스 위임 연결**을 켜고 같은 화면에서 필요한 **직무별 하네스 작업**을 설치하세요. 기존 설치의 연결·패키지 선택·사용자 유형은 재설치해도 유지됩니다.
...
GUI에서 **이력 수집 연결**을 선택하면 Claude의 `~/.claude/settings.json` 또는 Codex의 `~/.codex/hooks.json`에 WorkLog 훅을 추가합니다. **하네스 위임 연결**은 별도로 Claude의 `~/.claude/skills/work`, Codex의 `~/.agents/skills/work`와 `~/.codex/worklog/skills/work`를 연결합니다. 원래 설정은 백업하며 기존 사용자 훅·설정·동일 위치의 다른 스킬을 덮어쓰지 않습니다. Codex의 공식 자동 탐색은 `.agents/skills`를 사용하며 `.codex/worklog`는 같은 설치 원본의 참조 경로입니다. 기존 `CLAUDE.md`·`AGENTS.md`는 수정하지 않습니다. 연결 후 필요한 직무 패키지를 설치하면 해당 유형을 위임할 수 있습니다.
