...
작업 실행 설정은 **WorkLog 자동 생성**과 **하네스 작업**을 별도로 탐색한다. 앱의 자동 생성·GUI 작성 5종은 항상 설정할 수 있고, 하네스 유형은 설치한 직무 패키지에 따라 사용한다. 패키지 제거 후에도 사용자 유형과 저장한 지시문·모델 설정은 보존한다. 연결 설정에서 이력 수집과 하네스 위임을 독립 관리한다. [패키지와 자동 생성 관리](harness-packages.md)
...
기본 설정 대상은 모델 기반 업무 68개이며 제품·프로젝트·설계·FE·BE·개발 공통·조사 문서·시스템의 8개 그룹으로 표시한다. 등록한 사용자 유형은 별도의 **사용자 작업** 그룹에 표시한다. 로컬 검사 실행과 근거 보고서인 `checks.run`, `verification.report`에는 모델 설정이 없다. 기본 업무의 담당·제외 범위·필요 자료·완료 기준은 읽기 전용이며 지시문 편집으로 바뀌지 않는다.

작업 유형의 `worker_mode`와 단계별 도구 예산은 모델·effort와 별도인 실행 계약이다. `document.*` 4종과 `text.generate`는 제공 자료로 요청한 본문·수정본·검토 보고서를 직접 생성 1회와 코드 검사로 처리한다. 결과물의 별도 품질 검토는 추가로 요청한 경우에만 수행한다. 결정 기록·상태 보고·인수인계 등 필수 검토 유형은 직접 응답 방식을 사용해도 독립 검토를 유지한다. 직접 검토는 고정된 후보 본문과 코드 검사 결과를 받아 판정만 반환한다. 새 실행의 모델 worker timeout은 1시간이며 기존 실행 정의는 바꾸지 않는다. 전체 70종의 실행 방식과 정상·수정 호출 상한은 [작업 실행 행렬](task-execution-matrix.md)을 참고한다.
...
| `metadata` | 세션 요약, 제목·설명 재작성, 선택 날짜 업무 요약, 사용자 작업 유형 초안 | Luna / 모든 단계 high | Sonnet / 생성 low, 검토·수정 medium |
...
**작업 실행 설정 → 사용자 작업 등록**을 열면 먼저 새 작업을 설명하도록 안내한다. 목적과 대상, 입력 자료, 기대 결과, 수행 범위와 제외 사항, 품질 기준과 제약을 적고 **등록 내용 작성**을 누르면 템플릿·이름·용도·분류 키워드·Markdown 지시문을 채운 편집 화면으로 이동한다. 내용을 확인하고 Codex/Claude별 실행 설정을 선택한 뒤 **등록**을 눌러야 사용자 유형이 저장된다. **직접 입력**을 선택하면 기존 수동 등록 화면을 바로 사용할 수 있다.

초안 생성은 시스템 업무 **사용자 작업 유형 초안**(`task.type.draft`)이다. 기본 backend는 Codex, 모델은 `gpt-5.6-luna`, effort는 `high`이며 GUI에서 이 업무의 지시문·backend·model·effort를 별도로 설정할 수 있다. `metadata` 프로필과 `create-checked` workflow로 모델 생성 한 번과 JSON·템플릿·Markdown 코드 검사를 수행하고, 독립 모델 검토·자동 수정 루프는 수행하지 않는다. 이 설정은 등록 내용 작성에 적용되며 등록할 새 업무의 실행 설정과 구분된다. 작성 결과가 검사를 통과해도 실제 등록은 사용자 확인 후 별도로 수행한다.

문서·HTML·엔티티·코드 묶음·시나리오 계획을 생성하는 기본 업무를 템플릿으로 선택할 수 있다. 내부 메타데이터 작업, 로컬 검사 실행, 다른 사용자 유형은 템플릿으로 사용할 수 없다. 서비스가 현재 템플릿의 책임 경계와 기존 업무 목록을 제공하며 생성기는 허용된 템플릿 하나 안에서 초안을 작성한다. 지원 범위를 벗어나거나 서로 다른 전문 산출물이 섞인 요청은 범위 조정이 필요한 상태로 표시할 수 있다.

**요청 수정·다시 작성**에서 설명을 바꾸고 새 생성을 명시적으로 요청할 수 있다. 다시 작성하면 현재 초안을 새 결과로 바꾼다는 안내를 표시하며, 이전 초안으로 돌아갈 때 편집 내용을 유지한다. 실패·중단·범위 조정이 필요한 상태는 화면에 표시하고, 닫힌 화면이나 새 작성 요청보다 늦게 도착한 이전 응답으로 편집 내용을 덮어쓰지 않는다. 초안 작성은 실행 서비스에 기록하지만 새 업무 이력이나 사용자 유형을 자동 생성하지 않는다. [입력 안내·검사·API](custom-task-drafts.md)에 상세 계약을 정리했다.

사용자 유형에는 변경되지 않는 `user.<랜덤 ID>`를 부여한다. 이름과 용도, 키워드, 지시문, backend/model/effort는 편집할 수 있지만 입력 스키마, 산출물 파일·형식, 제외 범위, 검증 규칙과 workflow, worker 실행 방식은 선택한 템플릿에서 상속한다. 사용자 설명은 기존 책임 범위에 추가되며 제외 범위와 권한을 확대하지 않는다. 새 파일 형식이나 실행 명령이 필요하면 하네스 자체의 검증 계약을 구현해야 한다. 문서 4종·텍스트 작성·회의/진행 사실 요약 템플릿은 기본 생성 1회와 코드 검사, 명시적인 추가 품질 검토 선택 정책을 상속한다. 나머지 전문 유형은 템플릿의 필수 독립 검토를 유지한다.

등록 즉시 서비스의 `/catalog`와 `harness catalog --summary/--task`에 이름·설명·키워드·출처·템플릿·상속 계약이 포함된다. GUI의 **연결 설정**에서 해당 엔진에 연결한 요청 스킬은 이 카탈로그를 읽어 구조화된 계획의 task/input을 구성하므로 별도 업무별 스킬이나 앱 재설치가 필요하지 않다. 앱 설치만으로 Claude/Codex의 스킬·훅이 연결되지는 않는다. 자연어 `run`은 계속 단일·명확한 업무만 허용하며 모호한 요청을 임의 분류하지 않는다.
...
생성·검토·수정과 GUI 요약 등 모든 executor subprocess 및 CLI 버전 조회에는 `HARNESS_WORKER=1`을 전달한다. GUI에서 연결한 WorkLog 훅 명령은 이 값을 확인하면 Node를 실행하기 전에 성공 반환한다. 직접 `src/hook.mjs`를 실행하더라도 stdin을 읽거나 DB·스풀·오류 파일에 접근하기 전에 종료한다. 작업자 이력과 결과는 executor에서 직접 수집한다.
