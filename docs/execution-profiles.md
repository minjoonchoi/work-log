...
기본 설정 대상은 모델 기반 업무 67개이며 제품·프로젝트·설계·FE·BE·개발 공통·조사 문서·시스템의 8개 그룹으로 표시한다. 등록한 사용자 유형은 별도의 **사용자 작업** 그룹에 표시한다. 로컬 검사 실행과 근거 보고서인 `checks.run`, `verification.report`에는 모델 설정이 없다. 기본 업무의 담당·제외 범위·필요 자료·완료 기준은 읽기 전용이며 지시문 편집으로 바뀌지 않는다.
